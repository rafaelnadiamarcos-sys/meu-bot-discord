...
@bot.tree.command(name="ponto", description="Adiciona 1 ponto ao usuário.")
@app_commands.describe(usuario="Usuário a pontuar")
async def ponto(interaction: discord.Interaction, usuario: discord.User):
    if not somente_dono_slash(interaction):
        return await interaction.response.send_message("❌ Você não tem permissão.", ephemeral=True)
